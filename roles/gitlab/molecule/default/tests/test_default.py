...
def test_gitlab_nginx(host):
    ...
    url = host.ansible.get_variables()["gitlab_external_url"]
    response = host.ansible(
        "ansible.builtin.uri",
        f"url={url}/users/sign_in follow_redirects=none force=true return_content=true validate_certs=false",
        check=False,
    )

    assert response["status"] == 200
    assert "GitLab" in response["content"]


def test_registry_nginx(host):
    """Registry API returns 401."""
    url = host.ansible.get_variables()["gitlab_registry_external_url"]
    response = host.ansible(
        "ansible.builtin.uri",
        f"url={url}/v2/ follow_redirects=none force=true return_content=true validate_certs=false",
        check=False,
    )
...
def test_valid_root_user_password(host):
    ...
    script = login_test_script.format(
        url="https://git.example.com",
        user="root",
        password=password,
        expected_status=302,
    )
    result = host.run(f"echo '{script}' | python3")
